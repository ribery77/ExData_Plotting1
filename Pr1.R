#data <- read.csv("C:/RBook/EDA/household_power_consumption.txt", sep=";")
...
# check to see if the existing tidy data exists; if not, create it.
if (!file.exists('source data/power_consumption.txt')) {
        
        # download the zip file and unzip file
        ...
        # read the raw table and limit to 2 days period
        variable.class<-c(rep('character',2),rep('numeric',7))
        power.consumption<-read.table('source data/household_power_consumption.txt',header=TRUE,
                                      sep=';',na.strings='?',colClasses=variable.class)
        power.consumption<-power.consumption[power.consumption$Date=='1/2/2007' | power.consumption$Date=='2/2/2007',]
        
        # clean up the data and variable names and convert date/time fields into proper classes
